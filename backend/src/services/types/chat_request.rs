...
mod tests {
    ...
    use crate::common::types::message::{ChatCompletionRequestToolCall, ChatCompletionRequestFunctionCall};
    
    // Helper function to create a basic prompt with the given system and user templates
    fn create_test_prompt(system: &str, user: Option<&str>, prompt_type: &str) -> PromptRowWithModel {
        PromptRowWithModel {
            id: 1,
    ...
    fn test_new_with_missing_template_vars() {
        ...
        if result.is_err() {
            ...
            let service_request = result.unwrap();
            assert!(service_request.request.messages[0].content().contains("System prompt with ."));
        }
    }
    ...
    fn test_new_with_static_prompt_type() {
        ...
        assert_eq!(service_request.request.messages[0].content(), "Static system prompt.");
        assert_eq!(service_request.request.messages[1].content(), "User message");
    }
    ...
    fn test_new_with_dynamic_system_prompt_type() {
        ...
        assert!(service_request.request.messages[0].content().contains("System prompt with"));
        assert!(service_request.request.messages[0].content().contains("system value"));
    ...
    fn test_new_with_chat_mode() {
        ...
        assert_eq!(service_request.request.messages[1].content(), "User message 1");
        assert_eq!(service_request.request.messages[2].content(), "Assistant response");
        assert_eq!(service_request.request.messages[3].content(), "User message 2");
    }
    ...
    fn test_new_with_no_system_message() {
        ...
        assert_eq!(service_request.request.messages.len(), 4);
        assert!(service_request.request.messages[0].is_system());
        assert_eq!(service_request.request.messages[0].content(), "System prompt.");
    ...
    fn test_new_with_complex_conditional_template() {
        ...
        let system_content_1 = r#"{"detailed_mode": true, "include_pros_cons": true, "include_examples": false}"#;
        ...
        assert_eq!(service_request_1.request.messages[1].content(), "User message");
    }
    ...
    fn test_message_length_combinations() {
        ...
        let simple_prompt = create_test_prompt(
        ...
        assert!(service_request_case2.request.messages[0].content().contains("System template with system value"));
        ...
        assert!(service_request_case3.request.messages[0].content().contains("System template with system value"));
        ...
        assert!(service_request_case4.request.messages[0].content().contains("System template with system value"));
        ...
        assert!(service_request_case5.request.messages[0].content().contains("System template with system value"));
    }
}
