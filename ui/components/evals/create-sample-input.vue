    ...
    <form>
      <div class="space-y-12">
        <div class="grid grid-cols-1 gap-x-8 gap-y-10 border-b border-gray-900/10 pb-12 md:grid-cols-3">
          <div>
            <h2 class="text-base/7 font-semibold text-gray-900">Eval name</h2>
            <p class="mt-1 text-sm/6 text-gray-600">Give your eval a name that is meaningful and recognizable.</p>
          </div>
        ...
        <div class="grid grid-cols-1 gap-x-8 gap-y-10 border-b border-gray-900/10 pb-12 md:grid-cols-3">
          <div>
            <h2 class="text-base/7 font-semibold text-gray-900">Prompt fields</h2>
            <p class="mt-1 text-sm/6 text-gray-600">These are the variables in your prompt that you need to provide input for.</p>
          </div>
        ...
        <div class="grid grid-cols-1 gap-x-8 gap-y-10 border-b border-gray-900/10 pb-12 md:grid-cols-3">
          <div>
            <h2 class="text-base/7 font-semibold text-gray-900">Json input preview</h2>
            <p class="mt-1 text-sm/6 text-gray-600">The input that will be used for this eval based on prompt template variables.</p>
          </div>
          <div class="p-2 text-xs overflow-y-auto whitespace-pre h-32 grid max-w-2xl md:col-span-2 bg-neutral-100">
      ...
      <div class="mt-6 flex items-center justify-end gap-x-6">
        ...
        <PrimaryButton
          @click="handleSampleInput()"
          type="primary"
          size="sm"
          :disabled="sampleInputName === ''"
        >
          {{ sample ? 'Update' : 'Add' }} sample input
        </PrimaryButton>
      </div>
    </form>
    <div class="mt-6">
      <div class="px-4 sm:px-0">
        ...
        <p class="mt-1 text-sm/6 text-gray-600">We will substitute the variables that you populate above into your prompt at evaluation time.</p>
      </div>
...
import type { PromptSampleResponse } from '~/types/response/prompt_samples'
import { watch, ref, computed } from 'vue';

const props = defineProps<{
  prompt: Prompt,
  sample?: PromptSampleResponse | null
}>();

const emits = defineEmits(["cancel", "created", "updated"])

const { createSample, updateSample } = usePromptSamples();
...
watch(() => props.sample, (newSample) => {
  if (newSample) {
    sampleInputName.value = newSample.name;
    try {
      // Handle potential double-encoded JSON
      let inputData = JSON.parse(newSample.input_data);
