  ...
  <div class="font-mono">
    <form v-if="!modelsLoading">
              ...
              <div class="relative mt-2">
                ...
                >
                  <span class="col-start-1 row-start-1 flex w-full gap-2 pr-6">
                    <span class="truncate">{{ selectedModelId ? selectedModel?.model : 'Select a model' }}</span>
                    <span v-if="selectedModelId" class="truncate text-gray-500">{{ selectedModel?.provider }}</span>
                  </span>
                  <svg
                ...
                >
                  <li
                    ...
                    :class="{ 'bg-black text-white': model.id === selectedModelId }"
                    role="option"
      ...
      <div class="mt-6 flex items-center justify-end gap-x-6">
        <button @click="handlePromptCreation()" v-if="formIsValid" type="submit" class="text-sm/6 p-2 border-2 border-black">Update</button>
      </div>
    </form>
...
selectedModelId.value = props.prompt.model_id
...
const selectedModel = computed(() => {
  if (models.value && selectedModelId.value) {
    const model = models.value.filter(m => m.id === selectedModelId.value)
    return model[0]
  }
...
const selectModel = (model: number) => {
  selectedModelId.value = model;
  isOpen.value = false;
};


const promptUpdateLoading = ref(false)
async function handlePromptCreation() {
  promptUpdateLoading.value = true
