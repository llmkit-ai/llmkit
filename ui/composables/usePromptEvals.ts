...
export const usePromptEvals = () => {
  ...
  const fetchEvalById = async (id: number) => {
    try {
      loading.value = true
      currentEval.value = await $fetch<PromptEvalResponse>(`/v1/ui/prompt-evals/${id}`)
    } catch (err) {
      console.error(err)
  ...
  const fetchEvalByPrompt = async (promptId: number) => {
    try {
      loading.value = true
      evals.value = await $fetch<PromptEvalResponse[]>(`/v1/ui/prompts/${promptId}/prompt-evals`)
    } catch (err) {
      console.error(err)
  ...
  const createEval = async (sampleData: CreatePromptEvalRequest) => {
    try {
      const newEval = await $fetch<PromptEvalResponse>('/v1/ui/prompt-evals', {
        method: 'POST',
        body: sampleData
      })
  ...
  const updateEval = async (id: number, updates: UpdatePromptEvalRequest) => {
    try {
      const updatedEval = await $fetch<PromptEvalResponse>(`/v1/ui/prompt-evals/${id}`, {
        method: 'PUT',
        body: updates
      })
  ...
  const deleteEval = async (id: number) => {
    try {
      await $fetch(`/v1/ui/prompt-evals/${id}`, { method: 'DELETE' })
