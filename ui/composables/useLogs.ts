...
export const useLogs = () => {
  ...
  const fetchLogs = async (page: number = 1, pageSize: number = 20) => {
    try {
      loading.value = true
      logs.value = await $fetch<ApiLogReponse[]>('/api/v1/logs', {
        query: {
          page: page.toString(),
  ...
  const fetchLogById = async (id: number) => {
    try {
      loading.value = true
      log.value = await $fetch<ApiLogReponse>(`/api/v1/logs/${id}`)
    } catch (err) {
      console.error(err)
  ...
  const fetchLogsCount = async () => {
    try {
      loading.value = true
      const response = await $fetch<{ count: number }>('/api/v1/logs/count')
